...
class SubtitleList(YDLSerializable, BaseList[_T], Generic[_T]):
    ...
    def unique_by_language(self) -> Self:
        """
        Keep only the first subtitle encountered for each language family.
        Treats variations like 'en-orig', 'en-US', and 'en' as the same language.
        """
        seen_bases = set()
        items = []

        for sub in self:
            base_lang = sub.language.split("-")[0]

            if base_lang not in seen_bases:
                seen_bases.add(base_lang)
                items.append(sub)
    ...
    def sorted_by(
        ...
        if attribute == "best":
            filter = lambda s: (s.language, not s.autogenerated)
        else:
    ...
    def _from_ydl_dict(cls, data: dict) -> Self:
        ...
        def transform(entry: dict, autogenerated: bool = False):
            sub_type = "external" if ("url" in entry) else "inline"
            return {
                **entry,
                "type": sub_type,
                "language": lang,
