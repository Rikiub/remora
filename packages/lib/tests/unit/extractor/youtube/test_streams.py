...
async def test_audio_streams_validation(streams: StreamList):
    ...
    if itag_140:
        ...
        assert (codec := itag_140.audio.codec) and codec.original == "mp4a.40.2"
...
async def test_video_streams_validation(streams: StreamList):
    ...
    if itag_135:
        ...
        assert (codec := itag_135.video.codec) and codec.original == "avc1.4d401e"
...
async def test_muxed_streams_validation(streams: StreamList):
    ...
    if itag_18:
        assert (video_codec := itag_18.video.codec)
        assert (audio_codec := itag_18.audio.codec)
        assert video_codec.original == "avc1.42001E"
        assert audio_codec.original == "mp4a.40.2"
